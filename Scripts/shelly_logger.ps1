...
$OutFile = $OutFile = "C:\\Users\\5CG7471GSJ\\Documents\\DATA\\Load\\load1.txt"
...
function Get-NextTickUtc {
    ...
    $now = [DateTime]::UtcNow
    $seconds = [int][Math]::Floor(($now - $now.Date).TotalSeconds)
    $nextSeconds = ([int]([Math]::Floor($seconds / $IntervalSeconds)) + 1) * $IntervalSeconds
    return $now.Date.AddSeconds($nextSeconds)
}

function Sleep-UntilUtc {
    param([datetime]$TargetUtc)

    while ($true) {
        $remainingMs = [int][Math]::Ceiling(($TargetUtc - [DateTime]::UtcNow).TotalMilliseconds)
        if ($remainingMs -le 0) {
            break
        }

        if ($remainingMs -gt 1000) {
...
function Normalize-Status {
    ...
    $ts = $SampleTimeUtc.ToString("yyyy-MM-dd HH:mm:ss")
...
while ($true) {
    $sampleTime = Get-NextTickUtc -IntervalSeconds $PollIntervalSeconds
    Sleep-UntilUtc -TargetUtc $sampleTime
    ...
    try {
        ...
        foreach ($deviceId in $Devices.Keys) {
            ...
            $allRows += Normalize-Status -Payload $payload -DeviceName $deviceName -SampleTimeUtc $sampleTime
            Start-Sleep -Milliseconds 1200
        }

        Append-Rows -Rows $allRows -Path $OutFile
        Write-Host "$($sampleTime.ToString('yyyy-MM-dd HH:mm:ss')) UTC - logged $($allRows.Count) rows"
    }
    catch {
        Log-Error -Message $_.Exception.Message -Path $OutFile -SampleTimeUtc $sampleTime
