param(
    ...
    [switch]$RunOnce,
    [ValidateSet('all_on','whole_house','constant','constant_1kw','adaptive','all_off')]
    [string]$Scenario = 'constant',
    [string]$ScenarioRequestPath = (Join-Path $PSScriptRoot 'scenario_request.json'),
    [switch]$SetScenario,
    [switch]$SetScenarioAllOn,
    [switch]$SetScenarioWholeHouse,
    [switch]$SetScenarioConstant1kW,
    [switch]$SetScenarioAdaptive,
    [switch]$SetScenarioAllOff,
    [switch]$ShowScenario,
    [switch]$ShowStatus,
    [switch]$ClearScenarioRequest,
    [string]$StatusPath = (Join-Path $PSScriptRoot 'scenario_status.json'),
...
function ConvertTo-ScenarioName {
    ...
    switch ($Value.Trim().ToLowerInvariant()) {
        ...
        '3' { 'constant' }
        'constant' { 'constant' }
        'constant_1kw' { 'constant' }
        'constant1kw' { 'constant' }
        'constant-1kw' { 'constant' }
        '4' { 'adaptive' }
        'adaptive' { 'adaptive' }
        'all_off' { 'all_off' }
        'alloff' { 'all_off' }
...
function Resolve-RequestedScenario {
    $helperFlags = @(@(
        ...
        @{ Enabled = $SetScenarioConstant1kW; Value = 'constant' },
        @{ Enabled = $SetScenarioAdaptive; Value = 'adaptive' },
        @{ Enabled = $SetScenarioAllOff; Value = 'all_off' }
...
function Update-PendingScenario {
    ...
    try {
        ...
        if ($requested -eq 'constant' -and $null -ne $request.constant_target_w) {
            $requestedTarget = [double]$request.constant_target_w
            if ($requestedTarget -lt 0 -or $requestedTarget -gt 1800) {
                throw 'Requested constant target must be between 0 and 1800 W.'
            }
            $script:ConstantTargetW = $requestedTarget
        }
    }
    catch {
        Write-EventLog -Level 'WARN' -Event 'scenario_request_target_read_failed' -Message $_.Exception.Message
    }
...
$scenarioCommandRequested = $SetScenario -or $SetScenarioAllOn -or $SetScenarioWholeHouse -or $SetScenarioConstant1kW -or $SetScenarioAdaptive -or $SetScenarioAllOff
...
function Resolve-ConfigRelativePath {
    ...
    return Join-Path $configDirectory $Value
}

$script:LogPath = if ($config.event_log_path) { Resolve-ConfigRelativePath -Value ([string]$config.event_log_path) } else { Join-Path $configDirectory 'shelly_reconnect_events.tsv' }

if ($config.PSObject.Properties.Name -contains 'scenario_request_path' -and $config.scenario_request_path -and -not $PSBoundParameters.ContainsKey('ScenarioRequestPath')) {
    $script:ScenarioRequestPath = Resolve-ConfigRelativePath -Value ([string]$config.scenario_request_path)
}
if ($config.PSObject.Properties.Name -contains 'scenario_status_path' -and $config.scenario_status_path -and -not $PSBoundParameters.ContainsKey('StatusPath')) {
    $script:StatusPath = Resolve-ConfigRelativePath -Value ([string]$config.scenario_status_path)
}
